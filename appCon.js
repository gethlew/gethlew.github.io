...
class App{
    ...
    updateGamepadState(){
        ...
        //First input source (usually right hand controller)
        const inputSource = session.inputSources[0];

        //Asuming you're using a controller
        if (inputSource && inputSource.gamepad && this.gamepadIndices && this.ui
            && this.buttonStates){
                const gamepad = inputSource.gamepad;
                try{
                    //Searches through the button layout on controller
                    Object.entries( this.buttonStates ).forEach(( [key, value] ) => {
                        ...
                        if (key.indexOf('touchpad')!=-1 || key.indexOf('thumbstick')!=-1){
                            ...
                            //Gets value of thumbsticks's x axis
                            this.buttonStates[key].xAxis = 
                            gamepad.axes[xAxisIndex].toFixed(2);


                            //Gets value of thumbstick's y axis
                    ...
                    console.earn("An error occurred while searching UI");
                }
            } 
    }
    ...
    setupXR(){
        ...
        function onConnected( event ){
            ...
            fetchProfile( event.data, DEFAULT_PROFILES_PATH, DEFAULT_PROFILE
                ...
                //Returns simplified version of device profile
                info.name = profile.profileId;
                //Target ray mode is recieved whencontroller is connected
                info.targetRayMode = event.data.targetRayMode;

                //An array containing properties from the device's layout profile
                Object.defineProperties( profile.layouts ).forEach(( [key, layout] ) => {
                    //Array of buttons attached to controller
                    const components = {};
                    Object.values( layout.components ).forEach((component) => {
                        components[component.rootNodeName] =
                        component.gamepadIndicies;
                    });
                    //Array of all buttons attached to the controller
                    info[key] = components;
                });
